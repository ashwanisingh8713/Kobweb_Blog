...
suspend fun signUp(context: ApiContext) {
    try {
        ...
        val hashedUser = userRequest.copy(password = hashPassword(userRequest.password))
        val createdUser = context.data.getValue<MongoDB>().createUser(hashedUser)
        if (createdUser != null) {
            context.res.setBodyText(
                Json.encodeToString(
            ...
            context.res.setBodyText(Json.encodeToString("Username already exists."))
        }
    } catch (e: Exception) {
        context.res.setBodyText(Json.encodeToString(e.message))
    }
